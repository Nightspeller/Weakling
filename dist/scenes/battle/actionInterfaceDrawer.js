define(["require", "exports", "phaser", "../../config/constants", "../../entities/action"], function (require, exports, Phaser, constants_1, action_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    const { Rectangle } = Phaser.Geom;
    class ActionInterfaceDrawer {
        ...
        drawActionInterface() {
            return new Promise((resolve) => {
                ...
                availableActions.forEach((actionId) => {
                    const action = new action_1.default(actionId, currentCharacter);
                    ...
                    if (action.phase.includes(disposition.currentPhase)) {
                        ...
                        if (action.type === 'magical') {
                            buttonX = constants_1.GAME_W / 2 - 81 - 40 - 16 + (actionsOfType[1] % 3) * 81;
                            buttonY = 32 * Math.floor(actionsOfType[1] / 3);
                            actionsOfType[1] += 1;
                        }
                        if (action.type === 'misc') {
                            buttonX = constants_1.GAME_W - 32 - 3 * 81 + (actionsOfType[2] % 3) * 81;
        ...
        _drawActionDescription(action, show, x, y) {
            ...
            if (show) {
                ...
                const actionEffectsText = this.scene.add.text(x, lastTextY, `Effects: ${action.effects.map((effect) => effect.effectId)
                    .join(', ')}`, {
                    font: '12px monospace',
                    wordWrap: { width: 245 },
                    align: 'left',
    ...
    exports.default = ActionInterfaceDrawer;
});
