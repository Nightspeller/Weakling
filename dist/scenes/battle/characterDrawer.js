/* eslint-disable */
// @ts-nocheck
define(["require", "exports", "phaser", "../../characters/adventurers/adventurer", "../../config/constants", "../../helpers/progressBar"], function (require, exports, Phaser, adventurer_1, constants_1, progressBar_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    const { Rectangle } = Phaser.Geom;
    class CharacterDrawer {
        constructor(scene, char, positionIndex) {
            this.isParty = (char instanceof adventurer_1.default);
            ...
            const charInfoContainerX = this.position.x < 400
                ? this.position.x + constants_1.BATTLE_CHAR_WIDTH / 2
                : this.position.x - constants_1.BATTLE_CHAR_WIDTH / 2 - 32 * 8;
            ...
            this.makingTurnGraphics = this.scene.add.graphics()
                ...
                .strokeRectShape(new Rectangle(this.position.x - constants_1.BATTLE_CHAR_WIDTH / 2, this.position.y - constants_1.BATTLE_CHAR_HEIGHT / 2, constants_1.BATTLE_CHAR_WIDTH, constants_1.BATTLE_CHAR_HEIGHT))
                .setVisible(false);
        }
        initializeMainImage() {
        ...
        drawHealthAndManna() {
            ...
            const healthText = new progressBar_1.default({
                scene: this.scene,
                x: -constants_1.BATTLE_CHAR_WIDTH / 2,
                y: -constants_1.BATTLE_CHAR_HEIGHT / 2 - 36,
                color: 0xff0000,
                current: params.health,
                max: characteristics.health,
                width: constants_1.BATTLE_CHAR_WIDTH
            });
            const mannaText = new progressBar_1.default({
                scene: this.scene,
                x: -constants_1.BATTLE_CHAR_WIDTH / 2,
                y: -constants_1.BATTLE_CHAR_HEIGHT / 2 - 24,
                color: 0x0000ff,
                current: params.manna,
                max: characteristics.manna,
                width: constants_1.BATTLE_CHAR_WIDTH
            });
            const energyText = new progressBar_1.default({
                scene: this.scene,
                x: -constants_1.BATTLE_CHAR_WIDTH / 2,
                y: -constants_1.BATTLE_CHAR_HEIGHT / 2 - 12,
                color: 0x00ff00,
        ...
        drawActionPoints(show) {
            ...
            if (!show)
                return;
            const baseX = constants_1.BATTLE_CHAR_WIDTH / 2 - constants_1.ACTION_POINT_WIDTH + constants_1.ACTION_POINT_WIDTH / 2;
            const baseY = -constants_1.BATTLE_CHAR_HEIGHT / 2 + constants_1.ACTION_POINT_HEIGHT / 2;
            Object.keys(this.char.actionPoints)
                .forEach((pointType, index) => {
                let pointsDrawn = 0;
                const actionPointShift = constants_1.ACTION_POINT_HEIGHT * 2 * index;
                for (let i = 0; i < Math.min(Math.trunc(this.char.actionPoints[pointType]), 2); i += 1) {
                    this.actionPointsContainer.add(this.scene.add.sprite(baseX, baseY + pointsDrawn * constants_1.ACTION_POINT_HEIGHT + actionPointShift, 'action-points', index));
                    pointsDrawn++;
                }
                if (this.char.actionPoints[pointType] % 1 === 0.5) {
                    if (pointsDrawn < 2) {
                        this.actionPointsContainer.add(this.scene.add.sprite(baseX, baseY + pointsDrawn * constants_1.ACTION_POINT_HEIGHT + actionPointShift, 'action-points', index + 3));
                    }
                    else {
                        this.actionPointsContainer.add(this.scene.add.sprite(baseX + constants_1.ACTION_POINT_WIDTH, baseY + actionPointShift, 'action-points', index + 3));
                    }
                }
                if (this.char.actionPoints[pointType] === 3) {
                    this.actionPointsContainer.add(this.scene.add.sprite(baseX + constants_1.ACTION_POINT_WIDTH, baseY + actionPointShift, 'action-points', index));
                }
            });
        }
        drawEffectsIcons() {
        ...
        drawCharInfo() {
            ...
            let level;
            if (this.char instanceof adventurer_1.default) {
        ...
        playRangedProjectileAnimation(targetX, targetY) {
            return new Promise((resolve) => {
        ...
        playMissAnimation() {
            return new Promise((resolve) => {
                if (this.char.animations.miss) {
                    ...
                    const missText = this.scene.add.text(this.position.x, this.position.y, 'Miss!', { color: 'red', font: 'bold 30px monospace' })
                        .setOrigin(0.5, 0.5);
                    this.scene.tweens.add({
                        targets: missText,
                        props: {
            ...
            return new Promise((resolve) => {
                if (this.char.animations.death) {
                    ...
                    this.mainImage.setTexture('dead-character')
                        .setDisplaySize(constants_1.BATTLE_CHAR_WIDTH, constants_1.BATTLE_CHAR_HEIGHT);
                    resolve();
                }
            });
        }
    ...
    exports.default = CharacterDrawer;
});
