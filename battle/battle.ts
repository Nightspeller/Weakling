...
export class BattleScene extends Location {
    ...
    public drawDisposition(disposition: Disposition) {
        disposition.playerCharacters.forEach((char, index) => {
            ...
            if (!this.charImageMap.get(char)) this.charImageMap.set(char, this.add.container(position.x, position.y));
    ...
    public drawMakingTurnGraphics(char) {
        const charImageContainer = this.charImageMap.get(char);
        let makingTurnGraphics = charImageContainer.getByName('makingTurnGraphics') as Phaser.GameObjects.Graphics;
    ...
    private drawCharacter(char: GeneralEntity) {
        const imgParams = char.spriteParams;
        const charImageContainer = this.charImageMap.get(char);
        let charImage = charImageContainer.getByName('charImage') as Phaser.GameObjects.Sprite;
        if (!charImage) {
    ...
    private drawHealthAndManna(char) {
        const charImageContainer = this.charImageMap.get(char);
        const healthText = charImageContainer.getByName('healthText') as Phaser.GameObjects.Text;
        const mannaText = charImageContainer.getByName('mannaText') as Phaser.GameObjects.Text;
        const energyText = charImageContainer.getByName('energyText') as Phaser.GameObjects.Text;
        ...
        if (!healthText) {
            const healthText = this.add.text(
                0,
                -charHeight / 2 - 36,
                `${char.currentCharacteristics.parameters.currentHealth} / ${char.currentCharacteristics.parameters.health}`,
                {
                    font: '12px monospace',
                    fill: '#000000',
                    align: 'center',
        ...
        if (!mannaText) {
            const mannaText = this.add.text(
                0,
                -charHeight / 2 - 24,
                `${char.currentCharacteristics.parameters.currentManna} / ${char.currentCharacteristics.parameters.manna}`,
                {
                    font: '12px monospace',
                    fill: '#000000',
                    align: 'center',
        ...
        if (!energyText) {
            const energyText = this.add.text(
                0,
                -charHeight / 2 - 12,
                `${char.currentCharacteristics.parameters.currentEnergy} / ${char.currentCharacteristics.parameters.energy}`,
                {
                    font: '12px monospace',
                    fill: '#000000',
                    align: 'center',
    ...
    public drawActionPoints(char) {
        const charImageContainer = this.charImageMap.get(char);
        let actionPointsContainer = charImageContainer.getByName('actionPointsContainer') as Phaser.GameObjects.Container;
    ...
    private drawEffectsIcons(char) {
        const charImageContainer = this.charImageMap.get(char);
        let effectIconsContainer = charImageContainer.getByName('effectIconsContainer') as Phaser.GameObjects.Container;
        ...
        char.currentEffects.forEach((effect, index) => {
            ...
            iconSprite.setInteractive()
                ...
                .on('destroy', () => this.effectInformationGroup?.clear(true, true));
    ...
    private drawEffectInformation(effect: Effect, x: number, y: number) {
        ...
        this.effectInformationGroup.add(this.add.text(
            x + 8,
            y + 8,
            `${effect.name}`,
    ...
    private drawCharInfo(char: GeneralEntity) {
        ...
        const resistanceDetails = this.add.text(x + 8, y + 202,
            `🔥${char.currentCharacteristics.defences.fireResistance} ` +
    ...
    private playIdleAnimation(char: GeneralEntity) {
        const charImageContainer = this.charImageMap.get(char);
        const charImage = charImageContainer.getByName('charImage') as Phaser.GameObjects.Sprite;
        return new Promise((resolve, reject) => {
    ...
    public playDeathAnimation(char) {
        const charImageContainer = this.charImageMap.get(char);
        const charImage = charImageContainer.getByName('charImage') as Phaser.GameObjects.Sprite;
        return new Promise((resolve, reject) => {
    ...
    public playCastAnimation(char) {
        const charImageContainer = this.charImageMap.get(char);
        const charImage = charImageContainer.getByName('charImage') as Phaser.GameObjects.Sprite;
        return new Promise((resolve, reject) => {
            if (char.animations?.buff) {
    ...
    public drawTurnOrder(disposition: Disposition) {
    ...
    public drawActionInterface(disposition) {
